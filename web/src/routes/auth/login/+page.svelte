	...
	async function handleEmailLogin(e) {
		...
		try {
			...
			if (response.ok) {
				...
				error = data.error || $_('auth.loginFailed');
				console.log('Login error:', data.error, 'Status:', response.status);
			}
		} catch (err) {
			console.error('Login request error:', err);
			error = $_('auth.networkError');
		} finally {
			isLoading = false;
		}
	}
...
<svelte:head>
	<title>{$_('auth.login')} - Matcher</title>
</svelte:head>

<div class="min-h-screen bg-bg-dark flex items-center justify-center p-4">
	<div class="w-full max-w-md">
		<div class="text-center mb-8">
			<h1 class="text-4xl font-bold text-text-light mb-2">{$_('app.name')}</h1>
		...
		<div class="card p-6">
			<form on:submit={handleEmailLogin} class="space-y-4">
				...
				<div class="text-right">
					<a href="/auth/forgot-password" class="text-sm text-crimson-pulse hover:underline">
						Forgot my password
					</a>
				</div>
			...
			<div class="mt-6">
				...
				<a
					class="mt-4 w-full btn-rounded bg-white hover:bg-gray-100 text-gray-900 flex items-center justify-center gap-2"
			...
			<p class="mt-6 text-center text-sm text-text-light/60">
				{$_('auth.noAccount')} <a href="/auth/signup" class="text-crimson-pulse hover:underline">{$_('auth.signup')}</a>
			</p>
		</div>
	</div>
</div>
